...
- **Text Zoom:** Scale the editor content area (the note text) up or down with **⌘+ / ⌘- / ⌘0** on macOS (also a **Settings** stepper, which is how you zoom on iOS). The file list, window chrome, and toolbars stay at their native size.
...
- **Highlight Similar:** A toolbar toggle highlights every occurrence of the currently selected text across the open note (case-insensitive), updating live as the selection changes. The highlight color is configurable in **Settings**.
