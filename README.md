...
- **Collapsible Sidebar:** Toggle the note list (each row shows the note's name only, in a slightly larger bold font) using `Cmd-B` (also under the **View** menu). The list is ordered by modification date, with pinned notes hoisted to the top. **Reading a note never re-orders the list** — only a real edit moves a note, so browsing around doesn't shuffle what's under your cursor.
- **Opens Where You Left Off:** On launch DevNotes selects the note at the top of the (pin-first) list — or a **specific note you choose** in Settings ▸ On Open, which falls back to the top of the list if that note is later deleted — gives the editor keyboard focus so you can type immediately, and places the caret at the **first line**, **last line**, or **where you left off** (the note's last caret position, remembered per note — your choice in Settings). The editor scrolls to wherever the caret lands, on both platforms.
- **Search Jumps to the Line:** With a search query active, opening a note from the filtered list lands directly on the **first matching line** (match selected and scrolled into view) instead of the top of the note.
- **Heading Outline Panel:** Toggle a collapsible table of contents below the file list (sidebar toolbar button) built from the open note's `#`–`######` headings, nested by level. Click a heading to jump the caret straight to that line; click its chevron to collapse or expand just that branch.
...
- **Line Numbers Everywhere:** A **line-number gutter** on both macOS and iOS. Toggle it (and **Wrap Text**) from the **View** menu on macOS or from **Settings** on iOS. The gutter — like the current-line band and rule overlays — draws against only the visible portion of the note, so it stays smooth (and never disturbs the text) even on long documents.
- **Auto-Continuing Lists:** Press **Return** in a bullet or numbered item and the next line continues the list automatically; pressing Return on an empty item exits the list. Continuations are applied as a minimal in-place edit, so Return never resets the layout or jolts the scroll position.
...
- **Text Zoom:** Scale the editor content area (the note text) up or down with **⌘+ / ⌘- / ⌘0** on macOS, a **Settings** stepper, or the **zoom in/out buttons on the iOS toolbar**. The file list, window chrome, and toolbars stay at their native size.
- **Insert Date & Time:** Press **⌃⌥D** (or tap the **date/time button on the iOS toolbar**) to drop the current date and time at the caret. The format is a configurable `DateFormatter` pattern in **Settings** (default `yyyyMMdd-HHmmss`, e.g. `20260707-143022`).
- **One-Tap Backup:** **Settings ▸ Backup ▸ Create Backup** zips every note and exports it through the system save dialog (a save panel on macOS, Files on iOS) as `DevNotes-Backup-YYYYMMDD-HHMMSS.zip`. The archive **unzips to a folder of the same name**, and each note inside is filed under its **title** rather than the UUID it's stored as on disk — so a backup is readable without opening a single file. A `_manifest.txt` at the archive root maps every archived file back to its original on-disk name, which is what you'd restore it as to keep its pins, caret position, and open-on-launch setting. Notes sharing a title are numbered rather than overwritten. If a backup can't be built or written, DevNotes says why in an alert rather than failing quietly.
- **No Empty Files:** A note you empty out is **deleted from disk** (macOS deletes go to the Trash) instead of being saved as an empty file — typing again simply recreates it.
...
- **Basic Spell Checking:** Continuous, in-line spell checking (red underlines) — on by default and toggleable from the **View** menu or **Settings**. It never rewrites your text: automatic correction, text substitutions, grammar checking, and **smart-dash / smart-quote substitution** all stay off, so code and identifiers are left alone and typed Markdown matches its source — a typed `--` stays two hyphens (so `---` forms a real horizontal rule) and straight quotes never turn curly.
- **Word & Line Count:** A status bar pinned along the bottom of the editor shows a live **word count** and **line count** for the open note, updating as you type — on both macOS and iOS.
- **Follows the Caret:** The editor auto-scrolls to keep the cursor in view, so text typed at the bottom of the window is never clipped. Re-coloring is **incremental** — only the edited line is re-highlighted, so even long notes never blank out or drop keystrokes while typing — and both the syntax pass and the editor decorations are scoped to the visible viewport, so the view never jumps or jitters under your hands. Whenever the editor moves the view itself — pinning your position across a re-colour, or following the caret — it re-lays that viewport in the same pass, so a long note never shows a blank band waiting for the next scroll to fill it in. On iOS the editor **insets and scrolls itself clear of the on-screen keyboard** — re-measuring the overlap as the layout changes, so the inset always matches the actual keyboard coverage — and a configurable **bottom padding** (Settings, default 120pt) keeps scroll-past-end room so the caret on the last line never sits against the edge.
- **Export & Print:** From the **File** menu, export the open note as **Markdown** or **plain text**, **Save as PDF** (rendered with your editor styling), or **Create Backup…** to zip every note — the same backup this menu item and **Settings ▸ Backup** both build and save.
- **Regex Search:** Advanced search over the note list supporting regular expressions, whole-word filtering, and case-sensitivity. With a query active, each matching note appears as a **bold file name** with every occurrence listed below it — line number plus a snippet of the surrounding text — so you can jump straight to the exact line instead of just opening the note. A clear (**×**) button appears at the right edge of the search field whenever it has text — click it to clear the query and return to the full file list.
...
- **Ordered Lists Re-Number Themselves:** Moving a line up or down inside a numbered list re-sequences the markers, so shuffling items leaves them reading `1. 2. 3.` instead of out of order. Only the list you actually disturbed is rewritten; nested sub-lists number independently of their parent, a list that deliberately starts at `5.` keeps its start value, and the lazy all-`1.` Markdown style is left exactly as you wrote it.
- **Highlight Similar:** A toolbar toggle highlights every occurrence of the currently selected text across the open note (case-insensitive), updating live as the selection changes. The highlight uses a **separate colour for light and dark themes**, each configurable in **Settings**.
- **iPhone-Ready Editing:** A bold note-title bar (showing the note's **file name** in small text beside the title, when that name is meaningful) above an always-visible top bar (with roomier, ~20%-larger controls), the **outline formatting tools pinned on-screen** rather than hidden behind a sheet — the row scrolls sideways and adds **Undo**, **Insert Date & Time**, and **zoom in/out** buttons — a dedicated key to dismiss the keyboard, and note-list/settings sheets tuned for a phone-sized layout.
- **iPad Split-View Sidebar:** iPad gets the same collapsible `NavigationSplitView` sidebar as macOS — file list, search, and the heading outline panel all live in a persistent column beside the editor, toggled with a sidebar button in the toolbar — instead of iPhone's popup sheet. iPhone keeps the single-pane, sheet-based layout that fits its screen.
- **Toolbar Tooltips:** Every button on the outline toolbar, the sidebar toolbar, and the top bar shows its name on hover (macOS pointer, or iPadOS with a trackpad/mouse) via the standard `.help()` tooltip.
- **Long-Press to Open Links:** Press and hold any URL in a note to open it in your browser — a long press on iOS, a click-and-hold on macOS — on bare URLs and the address part of `[text](url)` links alike. Toggleable in **Settings** on both platforms (on by default). A plain click still places the caret and dragging still selects, so the gesture never gets in the way of editing.
...
- **Visual Conflict Merge:** Resolve sync conflicts using a side-by-side view on macOS and an inline view on iOS. The header shows the note's full on-disk path plus each side's originating device (**Mine** / **Theirs**), so it's clear exactly which file and which device is involved. Choosing a side clears the underlying file-version conflict on disk, so it can't resurface after a relaunch.
- **File Name in the Title:** A note's on-disk file name is shown alongside its title — as the **window subtitle** on macOS and in small text next to the title on iOS — but only when the name says something. Notes DevNotes creates are named after a UUID, so those show just the title (e.g. "Inbox"); a file you named yourself, such as `Inbox.md` dropped in via iCloud, still shows its name.
- **Version Indicator:** The current app version is shown in small text at the **bottom-left of the status bar** on both macOS and iOS.
- **Settings Follow You Across Devices:** Your **Editor Style** token sheet, theme, current-line and Highlight Similar colours, Insert Date & Time format, and On-Open jump all sync between Mac and iPhone/iPad through iCloud key-value storage — style the editor once and it looks the same everywhere, live (changes land while both apps are open, and on returning to the foreground). Genuinely device-specific preferences stay local: text zoom, wrap, line numbers, spell check, bottom padding, and the open-on-launch note.
- **Settings Conflicts Are Yours to Settle:** Each device stamps its settings with when they changed, so DevNotes can tell a change that genuinely arrived from another device from one of your own. If only one side changed, it's applied silently. If **both** devices changed their settings since they last agreed, DevNotes asks — a **Settings Differ Between Devices** alert naming what disagrees and which device the other copy came from, with **Use iCloud Settings**, **Keep This Device's**, or **Later**. Whichever you keep is published as the winner, so your other devices settle without asking you again.
...
A **DevNotes on GitHub** link sits at the top of Settings and again just above the Editor Style example, so the full token catalog below is always one click away from the box you're typing in.
...
| `bullet-color` | Hexadecimal color code (`#rgb`, `#rrggbb`, `#rrggbbaa`) | Color of bullet (`-`, `*`, `+`) and numbered-list markers (overrides the default blue). | `bullet-color: #ff9500` |
| `section-spacing` | Positive numeric values | Extra vertical space added **before each heading line**, so sections breathe without blank lines. | `section-spacing: 12` |
