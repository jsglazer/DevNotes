...
- **Follows the Caret:** The editor auto-scrolls to keep the cursor in view, so text typed at the bottom of the window is never clipped. Re-coloring is **incremental** — only the edited line is re-highlighted, so even long notes never blank out or drop keystrokes while typing — and never yanks the view. On iOS the editor **insets and scrolls itself clear of the on-screen keyboard**, so the lines you're typing stay visible instead of sliding behind it, and a configurable **bottom padding** (Settings, default 120pt) keeps scroll-past-end room so the caret on the last line never sits against the edge.
