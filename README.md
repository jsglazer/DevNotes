...
- **One-Tap Backup:** **Settings ▸ Backup ▸ Create Backup** zips every note and exports it through the system save dialog (save panel on macOS, Files on iOS) as `DevNotes-Backup-YYYYMMDD-HHMMSS.zip`.
...
- **iPhone-Ready Editing:** A bold note-title bar (showing the note's **file name** in small text beside the title) above an always-visible top bar (with roomier, ~20%-larger controls), the **outline formatting tools pinned on-screen** rather than hidden behind a sheet — the row scrolls sideways and adds **Undo**, **Insert Date & Time**, and **zoom in/out** buttons — a dedicated key to dismiss the keyboard, and note-list/settings sheets tuned for a phone-sized layout.
- **Long-Press to Open Links (iOS):** Long-press any URL in a note to open it in the browser — works on bare URLs and the address part of `[text](url)` links alike. Toggleable in **Settings** (on by default).
...
- **File Name in the Title:** The note's actual on-disk file name is shown alongside its title — as the **window subtitle** on macOS and in small text next to the title on iOS.
